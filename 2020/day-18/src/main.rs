...
static _SAMPLE_INPUT: &str = "1 + (2 * 3) + (4 * (5 + 6))
2 * 3 + (4 * 5)
5 + (8 * 3 + 9 + 3 * 4 * 3)
5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))
((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2";

static _INPUT: &str = "2 * (9 + 9 * (7 + 8 * 6 + 3 * 5) * 6 * 3) * ((6 + 4 * 3) + 9 + (9 * 8 * 6 * 7 * 8 * 9) * 2 * (6 + 2)) + 6
...
fn factor(tokens: &mut Peekable<Iter<char>>) -> u64 {
...
fn term(tokens: &mut Peekable<Iter<char>>) -> u64 {
    let mut val = factor(tokens);
...
fn expr(tokens: &mut Peekable<Iter<char>>) -> u64 {
    let mut val = term(tokens);
