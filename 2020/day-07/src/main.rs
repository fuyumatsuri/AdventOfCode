...
static _SAMPLE_INPUT: &str = "light red bags contain 1 bright white bag, 2 muted yellow bags.
...
static _SAMPLE_INPUT2: &str = "shiny gold bags contain 2 dark red bags.
...
static _INPUT: &str = "light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
type Bags = HashMap<String, Vec<Content>>;
type Content = (String, usize);

static TARGET_BAG: &str = "shiny gold";

fn search(bags: &Bags, contents: &[Content]) -> bool {
    contents
        .iter()
        .any(|content| content.0 == TARGET_BAG || search(&bags, &bags[&content.0]))
}

fn count(bags: &Bags, contents: &[Content]) -> usize {
    contents
        .iter()
        .map(|content| content.1 + content.1 * count(&bags, &bags[&content.0]))
        .sum()
}
...
fn main() {
    ...
    let bags = _INPUT
        ...
        .map(|line| {
            ...
            let contents = re2
                ...
                .map(|content| {
                    let count: usize = content.get(2).unwrap().as_str().parse().unwrap();
                    let name = content.get(3).unwrap().as_str().to_string();
    ...
    // Part 1
    // let count = bags.values().filter(|bag| {
    //     search(&bags, *bag)
    // }).count();

    // Part 2
    let count = count(&bags, &bags[TARGET_BAG]);
