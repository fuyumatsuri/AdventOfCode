...
static _SAMPLE_INPUT: &str = "L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
static INPUT: &str = "LLLL.LLLLL.LLLLLLLLLLLLLLLL.L.LLLLLL.LLLL..L.LLLLLLLL.LLLLLL.LLLL..LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
...
fn run(seats: &Vec<Vec<char>>) -> Vec<Vec<char>> {
    ...
    (0..width).into_iter().for_each(|i| {
        (0..height).into_iter().for_each(|j| {
            if seats[j][i] == 'L'
                && adjacent.iter().all(|(dir_x, dir_y)| {
                    let mut x = i as i32 + *dir_x;
                    let mut y = j as i32 + *dir_y;
                    while x >= 0
                        && x < width as i32
                        && y >= 0
                        && y < height as i32
                        && seats[y as usize][x as usize] == '.'
                    {
                        x = x as i32 + *dir_x;
                        y = y as i32 + *dir_y;
                    }
...
fn _print(seats: &Vec<Vec<char>>) {
    let str = seats.iter().map(|i| i.iter().join("")).join("\n");
    print!("{}\n\n", str);
}
...
fn main() {
    ...
    let mut old = seats.clone();
    let mut new = run(&seats);
    while old != new {
        old = new.clone();
        new = run(&new.clone());
    }
